...
func (s *Scheduler) GetRunner(c context.Context, m *Model, opts api.Options, sessionDuration *api.Duration) (chan *runnerRef, chan error) {
	...
	if runner != nil && !runner.needsReload(c, req) {
		slog.Info("[INSTRUMENT] GetRunner: fast path, reusing runner", "model", key)
		...
		runner.refMu.Unlock()
		if busy {
			slog.Warn("[INSTRUMENT] GetRunner: runner needs reload but busy, using fast path", "model", key, "refCount", runner.refCount)
			req.useLoadedRunner(runner, s.finishedReqCh)
		} else {
			slog.Info("[INSTRUMENT] GetRunner: slow path, queuing request", "model", key, "runnerNil", false)
			select {
		...
		slog.Info("[INSTRUMENT] GetRunner: slow path, queuing request", "model", key, "runnerNil", true)
		select {
		case s.pendingReqCh <- req:
		default:
			req.errCh <- ErrMaxQueue
			...
			for {
				...
				if runner != nil {
					if runner.needsReload(ctx, pending) {
						...
						runner.refMu.Unlock()
						if busy {
							slog.Warn("runner needs reload but has active requests, serving with current options to avoid scheduler deadlock", "model", pendingKey, "refCount", runner.refCount)
...
func (s *Scheduler) processCompleted(ctx context.Context) {
	...
	for {
		...
		case finished := <-s.finishedReqCh:
			...
			runner.refMu.Lock()
			runner.refCount--
			slog.Info("[INSTRUMENT] processCompleted: refCount--", "refCount", runner.refCount)
...
func (pending *LlmRequest) useLoadedRunner(runner *runnerRef, finished chan *LlmRequest) {
	...
	defer runner.refMu.Unlock()
	runner.refCount++
	slog.Info("[INSTRUMENT] useLoadedRunner: refCount++", "refCount", runner.refCount)
	if runner.expireTimer != nil {
		runner.expireTimer.Stop()
		runner.expireTimer = nil
	}
...
func (runner *runnerRef) needsReload(ctx context.Context, req *LlmRequest) bool {
	...
	defer cancel()
	adaptersChanged := !reflect.DeepEqual(runner.model.AdapterPaths, req.model.AdapterPaths)
	projectorsChanged := !reflect.DeepEqual(runner.model.ProjectorPaths, req.model.ProjectorPaths)
	optsChanged := !runner.model.IsMLX() && !reflect.DeepEqual(optsExisting, optsNew)
	pingErr := runner.llama.Ping(ctx)
	if adaptersChanged || projectorsChanged || optsChanged || pingErr != nil {
		slog.Info("[INSTRUMENT] needsReload: returning true",
			"adaptersChanged", adaptersChanged,
			"projectorsChanged", projectorsChanged,
			"optsChanged", optsChanged,
			"pingErr", pingErr,
			"optsExisting", fmt.Sprintf("%+v", optsExisting),
			"optsNew", fmt.Sprintf("%+v", optsNew))
		return true
	}
