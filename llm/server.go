...
func (s *llmServer) Completion(ctx context.Context, req CompletionRequest, fn func(CompletionResponse)) error {
	...
	slog.Info("[INSTRUMENT] Completion: acquiring sem")
	if err := s.sem.Acquire(ctx, 1); err != nil {
		slog.Info("[INSTRUMENT] Completion: sem acquire failed", "error", err)
	...
	slog.Info("[INSTRUMENT] Completion: sem acquired")
	...
	slog.Info("[INSTRUMENT] Completion: POSTing to runner", "endpoint", endpoint)
	res, err := http.DefaultClient.Do(serverReq)
	if err != nil {
		slog.Info("[INSTRUMENT] Completion: POST failed", "error", err)
	} else {
		slog.Info("[INSTRUMENT] Completion: POST response", "status", res.StatusCode)
	}
